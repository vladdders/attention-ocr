...
def var_random(name, shape, regularizable=False):
    ...
    v = tf.compat.v1.get_variable(
        name, shape=shape, initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"))
    if regularizable:
...
def ConvRelu(incoming, num_filters, filter_size, name):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        after_conv = tf.nn.conv2d(input=incoming, filters=conv_W, strides=(1, 1, 1, 1), padding='SAME')
...
def batch_norm(incoming, is_training):
    ...
    :return:
    '''
    return tf.compat.v1.layers.batch_normalization(incoming, training=is_training, scale=True)
...
def dropout(incoming, is_training, keep_prob=0.5):
    return tf.compat.v1.layers.dropout(incoming, rate=keep_prob, training=is_training)
